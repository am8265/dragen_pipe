...
## A global for the reanalysis scratch directory 
reanalysis_scratch = "/nfs/seqscratch_ssd/ALIGNMENT/BUILD37/DRAGEN"
...
class FixMergedMNPInfo(SGEJobTask):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.scratch_dir = "{0}/{1}/{2}.{3}".format(
        reanalysis_scratch,self.sample_type.upper(),self.sample_name,self.pseudo_prepid)        
        self.log_file = "{0}/logs/{1}.{2}.{3}.log".format(
        self.scratch_dir,self.sample_name,self.pseudo_prepid,self.__class__.__name__)
...
class AnnotateVCF(SGEJobTask):
    ...
    def work(self):
        ...
        self.temp_vcf = "{0}/{1}.{2}.analysisReady.fixed.vcf.tmp".format(
            self.scratch_dir,self.sample_name,self.pseudo_prepid)
        ...
        annotation_cmd = (""" /nfs/goldstein/software/jdk1.8.0_05/bin/java """
                          ...
                          """ GRCh37.87 {3} > {4} 2> {5} """.format(
                              config().clineff,config().clineff_cfg,config().annotatedbSNP,
                              self.fixed_vcf,self.annotated_vcf,self.log_file))
...
class DeAnnotateVCF(SGEJobTask):
    """ Run a custom script to strip annotations
    from a vcf file
    """
    ...
    def __init__(self, *args, **kwargs):
        super(DeAnnotateVCF,self).__init__(*args,**kwargs)
        self.scratch_dir = "{0}/{1}/{2}.{3}".format(
        reanalysis_scratch,self.sample_type.upper(),self.sample_name,self.pseudo_prepid)
        self.original_scratch_dir = "{0}/{1}/{2}.{3}".format(
        self.scratch,self.sample_type.upper(),self.sample_name,self.pseudo_prepid)
        
        if not os.path.exists(self.scratch_dir+'/logs'):
            os.makedirs(self.scratch_dir+'/logs')
        if not os.path.exists(self.scratch_dir+'/scripts'):
            os.makedirs(self.scratch_dir+'/scripts')
        ...
        self.deannotated_vcf_gz = "{0}/{1}.{2}.analysisReady.deannotated.vcf.gz".format(
            self.scratch_dir,self.sample_name,self.pseudo_prepid)
        self.annotated_vcf_gz = "{0}/{1}.{2}.analysisReady.annotated.vcf.gz".format(
            self.original_scratch_dir,self.sample_name,self.pseudo_prepid)
        try:
            db = get_connection("seqdb")
            cur = db.cursor()
            cur.execute(GET_PIPELINE_STEP_ID.format(
    ...
    def work(self):
        ...
        self.cmd = ("""{python} {deannotater} -o {deannotated_vcf} {vcf} &> {log}""".format(
                python=config().python_path,deannotater=config().deannotater,
    ...
    def requires(self):
        ...
        return MyExtTask(self.annotated_vcf_gz)
...
class RBP(SGEJobTask):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.scratch_dir = "{0}/{1}/{2}.{3}".format(
            reanalysis_scratch,self.sample_type.upper(),self.sample_name,self.pseudo_prepid)
        self.scratch_dir_original = "{0}/{1}/{2}.{3}".format(
            self.scratch,self.sample_type.upper(),self.sample_name,self.pseudo_prepid)
        self.log_file = "{0}/logs/{1}.{2}.{3}.log".format(
            self.scratch_dir,self.sample_name,self.pseudo_prepid,self.__class__.__name__)
        self.script = "{0}/scripts/{1}.{2}.{3}.sh".format(
            self.scratch_dir,self.sample_name,self.pseudo_prepid,self.__class__.__name__)
        self.recal_bam = "{0}/{1}.{2}.realn.recal.bam".format(
            self.scratch_dir_original,self.sample_name,self.pseudo_prepid)
        self.deannotated_vcf = "{0}/{1}.{2}.analysisReady.deannotated.vcf".format(
            self.scratch_dir,self.sample_name,self.pseudo_prepid)
        self.deannotated_vcf_gz = "{0}/{1}.{2}.analysisReady.deannotated.vcf.gz".format(
            self.scratch_dir,self.sample_name,self.pseudo_prepid)
...
class ArchiveSample(SGEJobTask):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.realn_bam = "{0}/{1}.{2}.realn.bam".format(
            self.scratch_dir,self.sample_name,self.pseudo_prepid)
        self.realn_bam_index = "{0}/{1}.{2}.realn.bam.bai".format(
            self.scratch_dir,self.sample_name,self.pseudo_prepid)
        self.annotated_vcf_gz = "{0}/{1}.{2}.analysisReady.annotated.vcf.gz".format(
            self.scratch_dir,self.sample_name,self.pseudo_prepid)
    ...
    def work(self):
        ...
        if not os.path.exists(self.gq_binned):
            os.makedirs(self.gq_binned)
        ...
        cmd = ("rsync --timeout=25000 -vrltgD "
               "{script_dir} {log_dir} {annotated_vcf_gz} "
               "{annotated_vcf_gz_index} "
               "{cvg_binned} {gq_binned} "
               "{variant_call_out} {geno_concordance_out} "
               "{contamination_out} {base_dir}").format(**self.__dict__)
        ...
        try:
            ...
            location = "{0}/{1}".format(self.scratch,self.sample_type.upper())
...
class CreateGenomeBed(SGEJobTask):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.seqtype_dir = os.path.join(reanalysis_scratch,self.sample_type.upper())
        self.sample_dir = os.path.join(self.seqtype_dir,self.sample_name+'.'+self.pseudo_prepid)
        self.seqtype_dir_original = os.path.join(self.scratch,self.sample_type.upper())
        self.sample_dir_original = os.path.join(self.seqtype_dir_original,self.sample_name+'.'+self.pseudo_prepid)
        kwargs["sample_name"] = self.sample_name
        super(CreateGenomeBed, self).__init__(*args, **kwargs)

        if not os.path.exists(self.sample_dir+'/logs'):
            os.makedirs(self.sample_dir+'/logs')
        if not os.path.exists(self.sample_dir+'/scripts'):
            os.makedirs(self.sample_dir+'/scripts')
        ...
        self.recal_bam = os.path.join(self.sample_dir_original,self.sample_name+'.{0}.realn.recal.bam'.format(self.pseudo_prepid))
...
class CvgBinning(SGEJobTask):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.binning_cmd = "{0} {1} 10000 {2} {3} {4} &> {5}".format(config().pypy,config().coverage_binner,self.sample_name+'.'+self.pseudo_prepid,self.genomecov_bed,self.output_dir,self.log_file)
    ...
    def work(self):
        ...
        self.remove_cmd = "rm -f {0}".format(self.genomecov_bed)
        run_shellcmd("seqdb",self.pipeline_step_id,self.pseudo_prepid,
...
class GQBinning(SGEJobTask):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.gvcf = os.path.join(self.sample_dir_original,self.sample_name+'.{0}.g.vcf.gz'.format(self.pseudo_prepid))
        self.log_dir = os.path.join(self.sample_dir,'logs')
...
class UpdateSeqdbMetrics(SGEJobTask):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.seqtype_dir = os.path.join(self.scratch,self.sample_type.upper())
        self.sample_dir = os.path.join(self.seqtype_dir,self.sample_name+'.'+self.pseudo_prepid)
        self.reanalysis_seqtype = os.path.join(reanalysis_scratch,self.sample_type.upper())
        self.reanalysis_sample = os.path.join(self.reanalysis_seqtype,self.sample_name+'.'+self.pseudo_prepid)
        kwargs["sample_name"] = self.sample_name
        super(UpdateSeqdbMetrics, self).__init__(*args, **kwargs)
        
        self.phased_vcf = "{0}/{1}.{2}.analysisReady.phased.vcf".format(
            self.reanalysis_sample,self.sample_name,self.pseudo_prepid)
        self.deannotated_vcf_gz = "{0}/{1}.{2}.analysisReady.deannotated.vcf.gz".format(
            self.reanalysis_sample,self.sample_name,self.pseudo_prepid)
        self.fixed_vcf = "{0}/{1}.{2}.analysisReady.fixed.vcf".format(
            self.reanalysis_sample,self.sample_name,self.pseudo_prepid)
        ...
        self.log_dir = os.path.join(self.reanalysis_sample,'logs')
        self.log_file = os.path.join(self.reanalysis_sample,
                                     self.sample_name+
        ...
        self.cvg_cs_out = os.path.join(self.sample_dir,"{0}.{1}.cvg.metrics.cs.txt".format(self.sample_name,self.pseudo_prepid))
        self.dup_out = os.path.join(self.sample_dir,"{0}.{1}.duplicates.txt".format(self.sample_name,self.pseudo_prepid))
        self.variant_call_out = os.path.join(self.reanalysis_sample,"{0}.{1}.variant_calling_summary_metrics".format(self.sample_name,self.pseudo_prepid))
        self.geno_concordance_out = os.path.join(self.reanalysis_sample,"{0}.{1}.genotype_concordance_metrics".format(self.sample_name,self.pseudo_prepid))
        self.contamination_out = os.path.join(self.reanalysis_sample,"{0}.{1}.contamination.selfSM".format(self.sample_name,self.pseudo_prepid))
    ...
    def work(self):
        ...
        for i in range(tries):
            try:
                ...
                if os.path.isfile(self.deannotated_vcf_gz):
                    subprocess.check_call(['rm',self.deannotated_vcf_gz,self.deannotated_vcf_gz+'.tbi'])                       
                ## Remove old fixed vcf
                if os.path.exists(self.fixed_vcf):
    ...
    def update_genotype_concordance_metrics(self):
        ...
        self.temp_genoconcordance = os.path.join(self.reanalysis_sample,"temp_geno_concordance.txt")
