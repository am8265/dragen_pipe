...
out_dir=os.path.join("/nfs/seqscratch09/jb3816/luigi_logs/",timestamp)
...
cmd="""PYTHONPATH='' luigi --module gatk_pipe ArchiveSample --sample-name {0} --pseudo-prepid {1} --capture-kit-bed  {2} --sample-type {3} --scratch {4} --poll-time 30 --workers 2 --worker-wait-interval 120 --scheduler-remove-delay 86400 > {5} 2>{6}""".format(chgvid,pseudo_prepid,capturekit_bed,seqtype,scratch,out_file,error_file)
print cmd
