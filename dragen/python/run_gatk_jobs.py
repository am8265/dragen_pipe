...
pseudo_prepid,chgvid,seqtype,capturekit_bed = info.split('\t')
now=datetime.now()
...
cmd="""luigi --module gatk_pipe ArchiveSample --sample-name {0} --pseudo-prepid {1} --capture-kit-bed  {2} --sample-type {3} --dont-remove-tmp-dir --no-tarball --poll-time 120 --workers 2 --worker-wait-interval 120 --scheduler-remove-delay 86400 > {4} 2>{5}""".format(chgvid,pseudo_prepid,capturekit_bed,seqtype,out_file,error_file)
