...
cmd="""luigi --module gatk_pipe UpdateSeqdbMetrics --sample-name {0} --pseudo-prepid {1} --capture-kit-bed  {2} --sample-type {3} --scratch {4} --dont-remove-tmp-dir --poll-time 30 --workers 2 --worker-wait-interval 120 --scheduler-remove-delay 86400 > {5} 2>{6}""".format(chgvid,pseudo_prepid,capturekit_bed,seqtype,scratch,out_file,error_file)
