...
DBuser = 'sequence_query'
DBpassword = 'sequence_query_pw'
database = 'sequenceDB'
...
if len(sys.argv) != 6:
    ...
    sys.exit(1)
scratch = sys.argv[1]                   
thresholdFile = sys.argv[2]             #Threshold values for relation to proband.
cryptic_threshold = float(sys.argv[3])  #This is currently set at 0.1
...
if testMode == 1:    # This mode updates the bioinfo database on sva9 and not the main SeqDB database
    print "SVA9 TEST MODE"
    print "you need to set non production connect info"
    sys.exit(1)
    DBhost = 'x'
    DBuser = 'x'
    DBpassword = 'x'
    DB = 'x'
    new_ped_location = scratch + "TESTDB/"
    master_ped = new_ped_location + "test"
...
# Convert to PED to BED using Plink
os.chdir(scratch)
