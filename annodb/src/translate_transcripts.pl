...
my $cfg = new Config::Simple("/nfs/goldstein/software/annodb/.my.cnf");

my $reg = 'Bio::EnsEMBL::Registry';
$reg->load_registry_from_db(
	-host => $cfg->param("clienthomo_sapiens_variation_73_37.host"),
	-user => $cfg->param("clienthomo_sapiens_variation_73_37.user"),
	-pass => $cfg->param("clienthomo_sapiens_variation_73_37.password"),
	-db_version => '73'
);

my $transcript_adaptor = $reg->get_adaptor('Human', 'Core', 'Transcript');
my $dbh = DBI->connect("DBI:mysql:homo_sapiens_core_73_37;mysql_read_default_file=/nfs/goldstein/software/annodb/.my.cnf;mysql_read_default_group=clienthomo_sapiens_variation_73_37", undef, undef);
my $query = "SELECT stable_id FROM transcript";
my $sth = $dbh->prepare($query);
$sth->execute();
my $results = $sth->fetchall_arrayref;
$sth->finish();
my $dbh2 = DBI->connect("DBI:mysql:homo_sapiens_variation_73_37;mysql_read_default_file=/nfs/goldstein/software/annodb/.my.cnf;mysql_read_default_group=clienthomo_sapiens_variation_73_37", undef, undef);
