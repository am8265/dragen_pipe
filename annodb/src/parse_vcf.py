...
DB_CONFIG_FILE = "/nfs/goldstein/software/dragen/.my.cnf"
SAMPLE_EXISTS_QUERY = """
SELECT 1
FROM sample
...
def insert_variant_entry(
    ...
    """
    cur.execute(VARIANT_INSERT.format(
        ...
        transcript_stable_id=escaped_value(rs_number),
    ...
    try:
        if anns:
            ...
            insert_variant_entry(
                ...
                transcript_stable_id=feature_id, effect=effect, HGVS_c=HGVS_c,
...
def get_variant_id(db, cur, CHROM, POS, REF, ALT, rs_number, ANNs):
    ...
    if row:
        ...
        return (insert_novel_variant(CHROM, POS, REF, alt, ALT, rs_number, ANNs),
                block_id)

def main(input_vcf_fh, output_directory, sample_id, log_fh):
    """parse the VCF, insert novel variants to the database,
    ...
    try:
        ...
        for line in input_vcf_fh:
            ...
            if fields["CHROM"] != current_CHROM:
                # close current calls file if it exists
                if current_CHROM in CHROMs:
                    calls_fh.close()
                # stop when we get to a chromosome not in the chromosome set
                current_CHROM = fields["CHROM"]
                if current_CHROM in CHROMs:
                    calls_fh = open(
                        "{output_directory}/called_variant_chr{CHROM}".format(
                            output_directory=output_directory,
                            CHROM=current_CHROM), "w")
                else:
                    break
            INFO_dict = create_INFO_dict(fields["INFO"])
            INFO_dict["PASS"] = 1 if fields["FILTER"] == "PASS" else 0
            ALT_alleles = fields["CHROM"].split(",")
            nalleles = len(ALT_alleles)
            variant_ids = []
            for ALT_allele in ALT_alleles:
                variant_ids.append(
                    get_variant_id(db, cur, CHROM, POS, REF, ALT,
            ...
            if nalleles == 1:
                ...
                for x, (variant_id, block_id) in enumerate(variant_ids):
                    calls_fh.write(VARIANT_CALL_FORMAT.format(
                        **merge_dicts(call, {"AD_ALT":ADs[1 + x]}, INFO_dict)) +
                        "\n")
        if current_CHROM in CHROMs:
            calls_fh.close()
    finally:
